...
@pytest.mark.parametrize(
    "release,expected",
    [
        ("2027.5", date(2027, 5, 1)),
        ("2027.12", date(2027, 12, 1)),
        ("2026.10.3", date(2026, 10, 1)),
        ("2027", None),
        ("", None),
        ("not.a.version", None),
        ("2027.13", None),
...
def test_release_estimated_date(release, expected):
    """Home Assistant ships in the first week, so the 1st is early but never
    late -- the right bias for a deadline warning."""
...
@pytest.mark.parametrize(
    ...
    [
        # Deadline 2027.5 lands ~2027-05-01.
        (date(2026, 8, 11), "2026.9", "upcoming"),   # 9 months out
        (date(2027, 3, 20), "2027.3", "upcoming"),   # 42 days out
        (date(2027, 4, 2), "2027.4", "imminent"),    # 29 days out
        (date(2027, 4, 30), "2027.4", "imminent"),   # 1 day out
...
def test_broken_now_is_decided_by_version_not_by_the_date_estimate(sample_index):
    ...
    # The estimate says the release is still 20 days away, but this system is
...
def test_imminent_carries_the_release_and_days(sample_index):
    ...
    assert report["imminent"] == {
        "fixture_tracker": {"release": "2027.5", "days": 20}
    }
    assert report["imminent_count"] == 1
    assert report["details"][0]["days_until"] == 20
